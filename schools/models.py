...
class Institution_Category(models.Model):
    ...
    class Meta:
        verbose_name = "Institution Categorie"
...
class Institution_Feature(models.Model):
    mgmt = models.ForeignKey(Institution_Management)
...
register_model(Institution_Feature)
...
class Institution_address(models.Model):
    ...
    class Meta:
        ...
        verbose_name = "Institution Addres"
...
class Boundary_Category(models.Model):
    ...
    class Meta:
        ...
        verbose_name = "Boundary Categorie"
...
class Boundary(models.Model):
    ...
    class Meta:
        ...
        """ Used For ordering """
        ordering = ['name']
        verbose_name = "Boundarie"

    def __unicode__(self):
        return '%s' % self.name

    def get_childobjects(self):
        dataList = []
        if Boundary.objects.filter(parent__id=self.id, active=2):
            dataList = Boundary.objects.filter(parent__id=self.id, active=2 )
        else:
            dataList = Institution.objects.filter(boundary = self)
        return dataList
...
class Institution(models.Model):
    ...
    name = models.CharField(max_length=300)
    cat = models.ManyToManyField(Institution_Category)
    ...
    def get_all_cat(self, category_type):
        return Institution_Category.objects.filter(institution = self)
    ...
    def get_fees_type_object(self):
        from fms_app.models import Fees_Type
        return Fees_Type.objects.filter(institution=self)
    
    def get_levels_object(self):
        from fms_app.models import Level
        return Level.objects.filter(institution=self)
    
    
    def get_student_fee_object(self):
        from fms_app.models import Student_fees
        return Student_fees.objects.filter(institution=self)
    
    def get_fine_type(self, ):
        from fms_app.models import Fine_type
        return Fine_type.objects.filter(institution=self)
    
    def get_fees_schedule(self, ):
        from fms_app.models import Fees_schedule
        return Fees_schedule.objects.filter(institution=self)
    

    def getStreams(self):
        return Stream.objects.filter(institution = self, active = 2)
...
state_choices = (('maharashtra','Maharashtra'),('karnataka', 'Karnataka'), ('andra_pradesh', 'Andra Pradesh'), ('telangana', 'Telangana'))
country_choices = (('japan','Japan'),('soviet', 'soviet'), ('nigeria', 'Nigeria'))
class Child(models.Model):
    ...
    address_line1 = models.TextField(max_length = 1000 , blank=True, null=True)
    address_line2 = models.TextField(max_length = 1000 , blank=True, null=True)
    state = models.CharField(max_length = 200 , choices = state_choices , blank=True, null=True)
    city = models.CharField(max_length = 200 , blank=True, null=True)
    country = models.CharField(max_length = 200 , blank=True, null=True)
    pin_code = models.CharField(max_length = 200 , blank=True, null=True)
    adhar_no = models.CharField(max_length = 50 , blank=True, null=True)
    election_id = models.CharField(max_length = 50 , blank=True, null=True)
    blood_group = models.CharField(max_length = 50 , blank=True, null=True)
    nationality = models.CharField(max_length = 200 , choices = country_choices , blank=True, null=True)
    physically_handicapped = models.BooleanField()
    religion = models.CharField(max_length = 200 , blank=True, null=True)
    student_photo = models.ImageField(upload_to = "." , verbose_name = "Upload Photo" , blank=True, null=True)
...
class Stream(models.Model):
    ...
    description = models.TextField("Stream description", blank=True, null=True)
    ...
    def get_studentgroups(self):
        groupsList=[]
        if StudentGroup.objects.filter(institution=self.institution, \
        stream = self, active=2):
            groupsList = \
                StudentGroup.objects.filter(institution=self.institution, \
                    stream = self, active=2 )
        return groupsList

    def getHOD(self):
        try:
            return Staff.objects.get(stream = self, staff_type__staff_type="HOD")
        except:
            pass

    def studentsCount(self):
        streamStudentsCount = 0
        studentgroupList = StudentGroup.objects.filter(stream = self, active = 2)
        for i in studentgroupList:
            if i.studentsCount():
                streamStudentsCount = streamStudentsCount + i.studentsCount()
        return streamStudentsCount
...
class StudentGroup(models.Model):
    ...
    section = models.CharField(max_length=10, choices=Alpha_list,
                               blank=True, default='')
    active = models.CharField(blank=True, null=True, default=2,max_length=10)
    group_type = models.CharField(max_length=10, choices=Group_Type,
                                  default='Class')

    class Meta:

        unique_together = ('institution','stream', 'name', 'section')
        ordering = ['name', 'section']

    def __unicode__(self):
        return '%s-%s' % ( self.stream.name , self.name)
    ...
    def getStaff(self):
        return Staff_StudentGroupRelation.objects.filter(student_group = self, staff__institution = self.institution, active = 2 )

    def getClassTeacher(self):
        try:
            ct =  ClassTeacher.objects.get(student_group = self, active = 2 , academic = current_academic())
        except:
            ct = ''
        return ct

    def studentsCount(self):
        return Student_StudentGroupRelation.objects.filter(student_group = self, active=2, academic=current_academic()).count()

    def getStudents(self):
        return Student_StudentGroupRelation.objects.filter(student_group = self, active=2, academic=current_academic())
...
class Student(models.Model):
    ...
    other_student_id = models.CharField(max_length=100, blank=True,
            null=True)
    active = models.IntegerField(blank=True, null=True, default=2)
    sms_features = models.BooleanField()
...
class Student_StudentGroupRelation(models.Model):
    ...
    class Meta:
        ...
        verbose_name = "Student StudentGroupRelation"
...
class Staff_StudentGroupRelation(models.Model):
    ...
    class Meta:
        ...
        verbose_name = "Staff StudentGroupRelation"
...
class Communication(models.Model):
    ...
    def __unicode__(self):
        return u'%s - %s' % (self.sender.username, self.teacher.email)

    def getStaffName(self):
        try:
            staffObj = Staff.objects.get(user = self.teacher)
        except:
            staffObj = ''
        return staffObj
    ...
    def getSenderName(self):
        try:
            staffObj = Staff.objects.get(user = self.sender)
        except:
            staffObj = ''
        return staffObj

    def studentComCount(self):
        return StudentCommunication.objects.filter(communication = self).count()
...
class StudentCommunication(models.Model):
    ...
    content_type = models.ForeignKey(ContentType,
                                     verbose_name=_('content type'),
                                     related_name='CCContentType')
    object_id = models.TextField(_('object ID'))
    created = models.DateField(blank=True, null=True)



class InstitutionPermissions(models.Model):
    institution = models.ForeignKey(Institution)
    user = models.ForeignKey(User)
    access = models.BooleanField(default=False)

    def __unicode__(self):
        return '%s - %s' %(self.institution.name, self.user.username)
